...
namespace CNB
{
    ...
    {
        public async static Task<LCommentsRaw> GetResults(string article_id)
        {
            var timstamp = ComputeMD5.GetTimeStop();
            var toBehashed = String.Format("app_key=10000&article={0}&format=json&method=phone.Comment&timestamp={1}&v=1.0&mpuffgvbvbttn3Rc", article_id, timstamp);
            ...
            var mylink = String.Format("http://api.cnbeta.com/capi?app_key=10000&article={0}&format=json&method=phone.Comment&timestamp={1}&v=1.0&sign={2}", article_id, timstamp, Md5);
